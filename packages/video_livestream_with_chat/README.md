...
2. **The channel reuses the call id.** `client.channel('livestream', id: call.id)` pairs the chat
...
3. **The player gives up its controls.** `LivestreamPlayer` keeps its backstage, ended and
...
4. **Chat is restyled for video, not screens.** A custom `messageBuilder` renders each message as
...
5. **The composer floats.** The `Scaffold` uses `resizeToAvoidBottomInset: false` so the video
   stays full-bleed, and the composer lifts itself over the keyboard using
   `MediaQuery.viewInsetsOf(context).bottom`.
6. **Reactions ride the Video API, not chat.** The heart and clap buttons call
...
> The host's camera needs a real camera. On a simulator the broadcast still goes live and chat and
> reactions work, but viewers see "The host's video is not available".
